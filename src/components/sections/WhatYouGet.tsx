export default function WhatYouGet() {
  return (
    <section id="what-you-get" className="relative py-20">
      <div className="container mx-auto flex max-w-7xl flex-col items-center gap-10 px-4">
        <h2 className="text-3xl font-bold tracking-tight text-slate-900 md:text-4xl">What You Get</h2>

        <div className="grid w-full grid-cols-1 gap-6 md:grid-cols-3">
          <div className="flex flex-col justify-between rounded-3xl bg-white shadow-xl shadow-slate-400/5">
            <div className="relative flex flex-1 items-center justify-center p-8 md:p-10">
              <img src="/images/what-you-get-1.png" className="relative z-10 shadow-xl shadow-slate-400/5" draggable="false"/>
              <img src="/images/grid-line.png" className="absolute inset-0" draggable="false"/>
            </div>

            <div className="p-7 text-lg font-medium text-slate-900 md:text-xl">A clear shortlist of technologies to evaluate.</div>
          </div>

          <div className="flex flex-col justify-between rounded-3xl bg-white shadow-xl shadow-slate-400/5">
            <div className="relative flex flex-1 items-center justify-center p-8 md:p-10">
              <img src="/images/what-you-get-2.png" className="relative z-10 shadow-xl shadow-slate-400/5" draggable="false"/>
              <img src="/images/grid-line.png" className="absolute inset-0" draggable="false"/>
            </div>

            <div className="p-7 text-lg font-medium text-slate-900 md:text-xl">Expert insight into why those tools fit your business.</div>
          </div>

          <div className="flex flex-col justify-between rounded-3xl bg-white shadow-xl shadow-slate-400/5">
            <div className="relative flex flex-1 items-center justify-center p-8 md:p-10">
              <img src="/images/what-you-get-3.png" className="relative z-10 shadow-xl shadow-slate-400/5" draggable="false"/>
              <img src="/images/grid-line.png" className="absolute inset-0" draggable="false"/>
            </div>

            <div className="p-7 text-lg font-medium text-slate-900 md:text-xl">Confidence you're only speaking with qualified vendors.</div>
          </div>
        </div>

        <a href="#" className="flex items-center gap-3 rounded-full border border-slate-300 bg-white/10 px-8 py-3 text-sm font-medium text-slate-900 backdrop-blur-xs duration-300 hover:bg-white/50 md:text-base">
          View GTM Marketplace
        </a>
      </div>
    </section>
  );
}
